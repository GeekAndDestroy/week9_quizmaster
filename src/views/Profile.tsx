...
import { CategoryType, EditUserFormType } from '../types';
...
export default function Profile({ logUserOut, flashMessage }: profileProps) {    
    ...
    const [editUserFormData, setEditUserFormData] = useState<EditUserFormType>(
        {
            first_name: '',
            last_name: '',
            email: '',
    ...
    const handleFormSubmit = async (e: React.FormEvent) => {
        ...
        let response = await editUser(localStorage.getItem("token")!, editUserFormData);
        if (response.error){
